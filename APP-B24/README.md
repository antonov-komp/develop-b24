...
│   ├── install.php                # Шаблон установки
│   └── config-error.php           # Шаблон ошибки конфигурации
├── tools/                         # Утилиты для отладки
│   ├── checkserver.php            # Проверка сервера
│   ├── test-*.php                 # Тестовые скрипты
...
Для включения внешнего доступа (без авторизации Bitrix24) добавьте в `config.json`:
```json
{
  "index_page": {
    "enabled": true,
