...
- ⚠️ **Рекомендация:** Убрать `error_reporting(E_ALL)` и `ini_set('display_errors', 1)` в продакшене
...
### 5. `auth-check.php` ⚠️ **УСТАРЕВШИЙ — ТРЕБУЕТ РЕФАКТОРИНГА**

**Назначение:** Функции для проверки авторизации Bitrix24 (обертки для обратной совместимости).

**Статус:** ⚠️ Устаревший, используется только в `install.php`

**Структура:**
- Функция `checkBitrix24Auth()` — обертка над `AuthService`
- Функция `redirectToFailure()` — обертка над `AuthService`

**Оценка:**
- ⚠️ **Использование:** Только в `install.php` для обратной совместимости
- ⚠️ **Проблема:** Дублирование функциональности `AuthService`
- ✅ **Рекомендация:** Рефакторить `install.php` для прямого использования `AuthService`, затем удалить этот файл

**Вывод:** Файл устаревший, используется только для обратной совместимости. Можно удалить после рефакторинга `install.php`.
...
**Структура:**
- Использует `auth-check.php` (устаревший)
- Использует `crest.php` для установки
- HTML-разметка встроена в PHP

**Оценка:**
- ✅ **Необходим** — требуется для установки приложения
- ⚠️ **Проблема:** Использует устаревший `auth-check.php`
- ⚠️ **Проблема:** HTML смешан с PHP (не соответствует MVC)
- ✅ **Рекомендация:** Рефакторить для использования `AuthService` и вынести HTML в шаблон
...
### 10. `checkserver.php` ⚠️ **УТИЛИТА — ОПЦИОНАЛЬНЫЙ**
...
- ⚠️ **Проблема:** Содержит секретные данные
- ✅ **Защищен:** В `.gitignore`, но не защищен `.htaccess` (PHP-файл)
- ⚠️ **Рекомендация:** Убедиться, что файл не коммитится в Git, рассмотреть защиту через `.htaccess`
...
**Структура:**
- Блокирует прямой доступ к `config.json` и `settings.json`
- Блокирует прямой доступ к `.log` файлам

**Оценка:**
- ✅ **Необходим** — защищает конфигурационные файлы
- ⚠️ **Проблема:** Не защищает `settings.php` (PHP-файл)
- ✅ **Рекомендация:** Добавить защиту для `settings.php` (если возможно)

**Вывод:** Файл необходим, но можно улучшить защиту `settings.php`.
...
| `auth-check.php` | ⚠️ Устаревший | ⚠️ Временно | Рефакторить `install.php`, затем удалить |
...
| `checkserver.php` | ⚠️ Утилита | ⚠️ Опционален | Оставить или переместить в `tools/` |
...
**Файл:** `auth-check.php`
- ⚠️ Используется только в `install.php`
- ✅ **Действие:** Рефакторить `install.php` для прямого использования `AuthService`, затем удалить `auth-check.php`

**Файл:** `install.php`
- ⚠️ Использует устаревший `auth-check.php`
- ⚠️ HTML смешан с PHP
- ✅ **Действие:** 
  - Использовать `AuthService` напрямую
  - Вынести HTML в шаблон `templates/install.php`
  - Создать `InstallController` (опционально)

### 3. Улучшить безопасность

**Файл:** `settings.php`
- ⚠️ Содержит секреты, но не защищен `.htaccess`
- ✅ **Действие:** Добавить защиту в `.htaccess` (если возможно) или переместить в защищенную директорию

**Файл:** `.htaccess`
- ⚠️ Не защищает `settings.php`
- ✅ **Действие:** Добавить правило для защиты `settings.php`:
  ```apache
...
**Файл:** `checkserver.php`
- ⚠️ Утилита для отладки в корне проекта
- ✅ **Действие:** Переместить в `tools/checkserver.php` или `scripts/checkserver.php`

### 5. Улучшить `index.php`

**Файл:** `index.php`
- ⚠️ Содержит `error_reporting(E_ALL)` и `ini_set('display_errors', 1)`
- ✅ **Действие:** Убрать в продакшене или вынести в конфигурацию:
  ```php
...
2. ✅ **Проверить безопасность** `settings.php` и `settings.json`

### Приоритет 2 (Важно)
3. ✅ **Рефакторить** `install.php` для использования `AuthService`
4. ✅ **Удалить** `auth-check.php` после рефакторинга `install.php`
5. ✅ **Улучшить** `.htaccess` для защиты `settings.php`

### Приоритет 3 (Желательно)
6. ✅ **Переместить** `checkserver.php` в `tools/` или `scripts/`
7. ✅ **Улучшить** `index.php` (убрать отладочные настройки в продакшене)
...
### ⚠️ Требуют внимания
1. **Устаревшие файлы:** `auth-check.php` (файл `access-control-functions.php` удален ✅)
2. **Безопасность:** `settings.php` требует дополнительной защиты
