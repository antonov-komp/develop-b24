...
**Дата завершения:** 2025-12-20 23:55 (UTC+3, Брест)  
**Статус:** Завершена  
**Приоритет:** Средний  
...
- [x] TASK-007 завершена (install.php использует AuthService напрямую) ✅
- [x] Файл `auth-check.php` удален ✅
- [x] Проверено, что файл нигде не используется (grep не находит упоминаний) ✅
- [x] Все функции из файла реализованы в `AuthService` ✅
- [x] Приложение работает корректно после удаления ✅
- [x] Все страницы открываются без ошибок:
  - [x] `index.php` работает ✅
  - [x] `install.php` работает (использует AuthService напрямую) ✅
...
- 2025-12-20 23:55 (UTC+3, Брест): Задача выполнена
  - ✅ Проверено завершение TASK-007 (install.php использует AuthService напрямую)
  - ✅ Проверено использование файла (не используется)
  - ✅ Проверена функциональность (все функции в AuthService)
