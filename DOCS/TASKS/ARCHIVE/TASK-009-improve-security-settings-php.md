...
**Дата завершения:** 2025-12-20 23:58 (UTC+3, Брест)  
**Статус:** Завершена  
**Приоритет:** Высокий  
...
- [x] Правило добавлено в `.htaccess` для защиты `settings.php` ✅
- [x] Прямой доступ к `settings.php` через браузер блокируется (403 Forbidden) ✅
- [x] Файл доступен через `require_once` в PHP-коде ✅
- [x] Приложение работает корректно после добавления защиты ✅
- [x] Все страницы открываются без ошибок:
  - [x] `index.php` работает ✅
  - [x] `install.php` работает ✅
...
- 2025-12-20 23:58 (UTC+3, Брест): Задача выполнена
  - ✅ Добавлено правило в `.htaccess` для защиты `settings.php`
  - ✅ Проверено использование файла (только через `require_once` в `crest.php`)
