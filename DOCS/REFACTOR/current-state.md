...
cd APP-B24
wc -c templates/index.php
wc -c templates/access-control.php
wc -c templates/token-analysis.php
```

**Время загрузки:**
- Зависит от скорости ответа Bitrix24 API
- Обычно 1-3 секунды для полной загрузки
- Время первого байта (TTFB): ~200-500ms
- Время рендеринга: ~500-1000ms
- Время загрузки API: ~500-2000ms

**Детали измерения:**
- Использовать Chrome DevTools Network tab
- Проверять Waterfall для каждого запроса
- Измерять время до First Paint
- Измерять время до Interactive
...
1. **Большой размер HTML:** Inline CSS увеличивает размер страницы
2. **Нет кеширования:** Каждый запрос загружает все заново
3. **Блокирующие запросы:** API запросы блокируют рендеринг
4. **Нет оптимизации изображений:** Фото пользователей не оптимизированы
...
**Детали ручного тестирования:**
1. **Тестирование главной страницы:**
   - Открыть приложение в Bitrix24
   - Проверить отображение данных пользователя
   - Проверить работу кнопок
   - Проверить обработку ошибок

2. **Тестирование управления правами:**
   - Открыть страницу управления правами
   - Добавить отдел
   - Добавить пользователя
   - Удалить отдел/пользователя
   - Переключить проверку прав

3. **Тестирование анализа токена:**
   - Открыть страницу анализа
   - Проверить отображение данных
   - Проверить копирование JSON

**Проблемы текущего подхода:**
- Трудоемкость: каждое изменение требует ручного тестирования
- Риск пропустить баги: не все сценарии проверяются
- Нет регрессионного тестирования: сложно проверить, что ничего не сломалось
- Нет автоматизации: нельзя запустить тесты в CI/CD

**Что нужно добавить:**
- Unit-тесты для сервисов (PHPUnit)
- Интеграционные тесты для API
- E2E тесты для Vue.js компонентов (если будет Vue.js)
...
**Зависимости:**
- LoggerService → нет зависимостей
- ConfigService → зависит от LoggerService
- Bitrix24Client → зависит от LoggerService
- Bitrix24ApiService → зависит от Bitrix24Client, LoggerService
- UserService → зависит от Bitrix24ApiService, LoggerService
- AccessControlService → зависит от ConfigService, Bitrix24ApiService, UserService, LoggerService
- AuthService → зависит от ConfigService, AccessControlService, Bitrix24ApiService, UserService, LoggerService

**Порядок инициализации важен!** Сервисы должны инициализироваться в порядке зависимостей.

### Текущая структура контроллеров

**Базовый контроллер:** `APP-B24/src/Controllers/BaseController.php`
- Метод `render()` для отображения шаблонов
- Обработка ошибок
- Логирование

**Контроллеры:**
- `IndexController` — главная страница
- `AccessControlController` — управление правами
- `TokenAnalysisController` — анализ токена

**Все контроллеры:**
- Наследуются от BaseController
- Получают сервисы через конструктор (Dependency Injection)
- Используют шаблоны из `templates/`
...
- **2025-12-20 20:15 (UTC+3, Брест):** Добавлены детали производительности, детали тестирования, детали структуры файлов
